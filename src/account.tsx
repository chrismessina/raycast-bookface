    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          {me.email ? <Detail.Metadata.Label title="Email" text={me.email} /> : null}
          <Detail.Metadata.Label title="User ID" text={String(me.id)} />
          {companies.length > 0 ? (
            <Detail.Metadata.TagList title="YC Companies">
              {companies.map((c) => (
                <Detail.Metadata.TagList.Item key={c.id} text={`${c.name} (${c.batch})`} />
              ))}
            </Detail.Metadata.TagList>
          ) : null}
          <Detail.Metadata.Separator />
          <Detail.Metadata.Link title="Profile" target={profileUrl} text="View on Bookface" />
        </Detail.Metadata>
      }
      actions={
        <ActionPanel>
          <Action.OpenInBrowser title="Open Profile in Browser" url={profileUrl} />
